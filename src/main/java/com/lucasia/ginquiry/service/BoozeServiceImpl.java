...
public class BoozeServiceImpl implements BoozeService {
    ...
    public Brand findOrSaveBrand(Brand brand) {
        ...
        final Brand findByNameBrand = brandRepository.findByName(brand.getName()); // check the db if Brand exists

        if (findByNameBrand != null) return findByNameBrand;

        // save a new one
        final Brand newBrand = brandRepository.save(brand);
        brandRepository.flush();    // explicitly flush so Booze can save with the persisted Brand
        return newBrand;
